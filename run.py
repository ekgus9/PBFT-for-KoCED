...
from src.dataset_koced import FewShotDataset
from src.models import BertForPromptFinetuning, RobertaForPromptFinetuning, resize_token_type_embeddings, BartForPromptFinetuning, XLMRobertaForPromptFinetuning
...
def main():
    ...
    logger.info("number of labels: {}, output mode: {}".format(num_labels, output_mode))
    ...
    if 'prompt' in model_args.few_shot_type:
        if config.model_type == 'roberta':
            ...
            model_fn = BartForPromptFinetuning
    ...
    if data_args.prompt:
        model.label_word_list = torch.tensor(train_dataset.label_word_list).long().to(training_args.device) #.cuda()
    ...
    def build_compute_metrics_fn():
        def compute_metrics_fn(p: EvalPrediction):
            ...
            # f1
            pred_tags, gold_tags = preds, label_ids
            ...
            try: mcc = mcc_numerator / mcc_denominator
            except: mcc = -1
            
            print(f"acc: {acc} mcc: {mcc} f1-not: {f1_all_scores[0]} f1-err: {f1_all_scores[1]} f1-multi: {f1_all_scores[0]*f1_all_scores[1]}")
            return metrics
            ...
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    ...
                    for key, value in test_result.items():
                        logger.info("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
                        final_result['_test_' + key] = value

                if training_args.save_logit:
                    predictions = output.predictions
                    num_logits = predictions.shape[-1]
                    logits = predictions.reshape([test_dataset.num_sample, -1, num_logits]).mean(axis=0)
                    np.save(os.path.join(training_args.save_logit_dir, "{}-{}.npy".format(training_args.model_id, training_args.array_id)), logits)
