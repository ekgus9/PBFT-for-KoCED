...
import json
from src.processors import median_mapping
...
def input_example_to_string(example, sep_token):
    ...
    elif example.text_c is None:
        # Warning: very simple hack here
        return example.text_a + ' ' + sep_token + ' ' + example.text_b
    else:
        return example.text_a + ' ' + sep_token + ' ' + example.text_b + ' ' + sep_token + ' ' + example.text_c
...
class KocedProcessor(DataProcessor):
...
class Koced_G_Processor(DataProcessor):
    ...
    def get_example_from_tensor_dict(self, tensor_dict):
        ...
        return InputExample(
            ...
            tensor_dict["s1"].numpy().decode("utf-8"),
            tensor_dict["s2"].numpy().decode("utf-8"),
            tensor_dict["google"].numpy().decode("utf-8"),
            str(tensor_dict["label"].numpy()),
        )
